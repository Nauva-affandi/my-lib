...
export class router {
  ...
  init() {
    try {
      window.addEventListener(this.mode === 'history' ? 'popstate' : 'hashchange', () => this.resolveRoute())
  ...
  findRoute(path) {
    try {
      return this.routes.find((r) => r.path === path || (r.path.includes(':') && this.matchDynamicRoute(r.path, path)))
    } catch (err) {
  ...
  extractParams(routePath, currentPath) {
    try {
      ...
      let paramRegex = new RegExp(routePath.replace(/:\w+/g, '([\\w-]+)'), 'i')
